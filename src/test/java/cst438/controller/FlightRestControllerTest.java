...
public class FlightRestControllerTest {
   ...
   public void getFlightInfo() throws Exception {
      ...
      MockHttpServletResponse response = mvc.perform(
            get("/api/flight/v1/AS1922").contentType(MediaType.APPLICATION_JSON).content(json.write(flightInfo).getJson()))
   ...
   public void getAllFlightsInfo() throws Exception {
      ...
      List<FlightInfo> flights = Arrays.asList(new FlightInfo(1, "AS1922","SFO","LAX","2020-05-01 08:30:00","2020-05-01 10:30:00", 
            "San Francisco International Airport", "San Francisco, California, United States", 
            "Los Angeles International Airport", "Los Angeles, California, United States"),
      ...
      //stub out Flight Service class
      given(flightService.getAllFlights("SFO", "LAX")).willReturn(flights);
      
      //when
      MockHttpServletResponse response = mvc.perform(get("/api/flights/v1/SFO/LAX").contentType(MediaType.APPLICATION_JSON)
            .content(listJson.write(flights).getJson())).andReturn().getResponse();
   ...
   public void getAllFlightsInfoNotFoundAndReturnEmptyList() throws Exception {
      ...
      given(flightService.getAllFlights("AAA", "BBB")).willReturn(Collections.emptyList());

      // when
      ResultActions response =  mvc.perform(get("/api/flights/v1/AAA/BBB"));
